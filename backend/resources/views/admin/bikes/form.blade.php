@extends('layouts.admin', ['title' => $bike->exists ? 'Edit Bike' : 'Tambah Bike'])

@section('content')
    <h1>{{ $bike->exists ? 'Edit Bike' : 'Tambah Bike' }}</h1>
    <div class="card">
        <form method="post" action="{{ $bike->exists ? route('admin.bikes.update', $bike) : route('admin.bikes.store') }}">
            ...
            <select name="status">
                @foreach(['available','reserved','in_use','idle','offline','maintenance'] as $status)
                    <option value="{{ $status }}" @selected(old('status', $bike->status) === $status)>{{ $status }}</option>
                @endforeach
            </select>
